	...
	while (true) {
		...
		while (true) {
			if (triger)
			{
				...
				{
					if (is_already.type == Timer_Event::TIMER_TYPE::TYPE_PING_TEST)
					{
						WSA_OVER_EX* wsa_ex = new WSA_OVER_EX;
						wsa_ex->setCmd(CMD_PING_TEST);
						PostQueuedCompletionStatus(hiocp, 1, is_already.server_id, &wsa_ex->getWsaOver());
					}
					triger = false;
			...
			{
				if (exec_event.type == Timer_Event::TIMER_TYPE::TYPE_PING_TEST)
				{
					WSA_OVER_EX* wsa_ex = new WSA_OVER_EX;
					wsa_ex->setCmd(CMD_PING_TEST);
					PostQueuedCompletionStatus(hiocp, 1, exec_event.server_id, &wsa_ex->getWsaOver());
				}
			}
			else {
